...
@section('content')
    <div class="card p-3">
        <h3 class="text-center mb-5 fw-bold">Tambah Data Sidik Jari</h3>
        <form action="{{ route('fingerprint.store') }}" method="POST">
            ...
            <select class="form-control" id="vote_id" name="vote_id">
                ...
                @foreach ($votepapper as $papper)
                    <option value="{{ $papper->vote_id }}">{{ $papper->vote_id }} -- {{ $papper->periode }}</option>
                @endforeach
            </select>
            <br>
            <div class="mb-3">
                <label for="template" class="form-label">Template Sidik Jari</label>
                <textarea type="text" class="form-control" id="template" value="" name="template" readonly></textarea>
            </div>
